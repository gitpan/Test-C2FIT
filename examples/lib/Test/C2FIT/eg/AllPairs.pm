...
sub toString {  # TBD: overload stringify operator?
    my $self = shift;
    my $ls = (defined($self->{left}))  ? $self->{left}->toString  : "null";
    my $rs = (defined($self->{right})) ? $self->{right}->toString : "null";
...
sub doAllItems {
    ...
    for(my $i = 0; $i < @$files; $i++) {
        my $item = new Test::C2FIT::eg::AllPairs::Item($var,$i,$self->{rank}++);
        $toItem->{$files->[$i]} = $item;
        push(@{$var->{items}},$item);
    }
}
...
sub doAllItemPairs {
    ...
    for(my $l=0; $l < $vl->size; $l++) {
        for(my $r=0; $r < $vr->size; $r++) {
            push(@{$self->{pairs}}, new Test::C2FIT::eg::AllPairs::Pair(\$self->{rank}, $vl->get($l),$vr->get($r)));
        }
    }
}
...
sub getFirstPair {  # java uses a sorted collection, in the perl port pairs are sorted on demand
    my $self = shift;
    my $pairs = $self->{pairs};
